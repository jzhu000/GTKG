...
def construct_temporal_relations(dataset, file):
    ...
    save_dir = os.path.join(root_dir, f'data/tem-{dataset}/add_rels/gpt/')
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    # files = ['train_rest_cand.json', 'dev_rest_cand.json', 'test_rest_cand.json']
    
    prompt = open("codes/prompt.txt").read().strip()
    error_data = []
    ...
    for sample in raw_data:
        ...
        for sentences in sample["cand_temporal_relation"]:
            ...
            try:
                ans = GptAPI(sys_pmt=sys_prompt, user_pmt=user_prompt)
                print(ans)
                ...
                if max_score >= 0.8 and score_count[max_score] == 1:
